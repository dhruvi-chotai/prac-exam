...
pipeline {
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'master', credentialsId: 'git-credentials', url: 'https://github.com/dhruvi-chotai/prac-exam.git'
            }
        }
        stage('init'){
            steps{
                script{
                    ...
                    sh "git clone https://github.com/dhruvi-chotai/prac-exam.git"
                }
            }
        }
        stage('config'){
        ...
        stage('build') {
            ...
            steps {
                script{
                    ...
                    sh "docker build -t dhruvi0911/prac-exam:${IMAGE_NAME} ."    
                    }
            }
        }
      stage('test') {
        ...
        // }
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'docker', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]){
                    ...
                    sh "docker push dhruvi0911/prac-exam:${IMAGE_NAME}"
                }}
        ...
        stage('deploy'){
            steps{
                script{
                    ...
                    def dockerRunCmd = "sudo docker run -p 8080:8080 -d dhruvi0911/prac-exam:${IMAGE_NAME}"
                  sshagent(['ec2-prod']) {
